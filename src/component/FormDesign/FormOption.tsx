...
export const formOption = (
  ...
  main: ZeFormItem,
  opt: ZeFormItem[],
  setMainItems: React.Dispatch<React.SetStateAction<ZeFormItem[]>>
) => {
  key.forEach((k) => {
    if (k === "label")
      main.type !== "list"
      ...
      opt.push(
        template(
          "text",
          "默认值",
          `${main.item?.name}_initialValue`,
          main.item?.initialValue,
          (e) =>
            setMainItems((v) => {
      ...
      opt.push(
        template(
          "text",
          "必需校验信息",
          `${main.item?.name}_rules`,
          main.item?.rules?.[0]?.["message"],
          (e) =>
            setMainItems((v) => {
              v[index].item!.rules = e.target.value
                ? [{ required: true, message: e.target.value }]
                : [{ required: false }];
              return [...v];
            })
        )
      ...
      opt.push(
        template(
          "switch",
          "显示字数",
          `${main.item?.name}_showCount`,
          main.option.showCount,
          (e) =>
            setMainItems((v) => {
      ...
      opt.push(
        template(
          "number",
          "最大字数",
          `${main.item?.name}_maxLength`,
          main.option.maxLength,
          (e) =>
            setMainItems((v) => {
      ...
      opt.push(
        template(
          "radio",
          "选项类型",
          `${main.item?.name}_optionType`,
          main.option.optionType,
          (e) =>
            setMainItems((v) => {
      ...
      opt.push(
        template(
          "switch",
          "允许半星",
          `${main.item?.name}_allowHalf`,
          main.option.allowHalf,
          (e) =>
            setMainItems((v) => {
      ...
      opt.push(
        template(
          "switch",
          "可查询",
          `${main.item?.name}_showSearch`,
          main.option.showSearch,
          (e) =>
            setMainItems((v) => {
      ...
      opt.push(
        template(
          "switch",
          "可清除",
          `${main.item?.name}_allowClear`,
          main.option.allowClear,
          (e) =>
            setMainItems((v) => {
      ...
      opt.push(
        template(
          "switch",
          "初始是否选中",
          `${main.item?.name}_defaultChecked`,
          main.option.defaultChecked,
          (e) =>
            setMainItems((v) => {
      ...
      opt.push(
        template(
          "switch",
          "显示时间 showTime",
          `${main.item?.name}_showTime`,
          main.option.showTime,
          (e) =>
            setMainItems((v) => {
      ...
      opt.push(
        template(
          "number",
          "分片文件大小",
          `${main.item?.name}_chunkSize`,
          main.option.chunkSize,
          (e) =>
            setMainItems((v) => {
      ...
      opt.push(
        template(
          "number",
          "获取 token 方式",
          `${main.item?.name}_tokenType`,
          main.option.tokenType,
          (e) =>
            setMainItems((v) => {
      ...
      opt.push(
        template(
          "radio",
          "上传列表样式",
          `${main.item?.name}_listType`,
          main.option.listType,
          (e) =>
            setMainItems((v) => {
      ...
      opt.push(
        template(
          "number",
          "文件数量",
          `${main.item?.name}_maxCount`,
          main.option.maxCount,
          (e) =>
            setMainItems((v) => {
      ...
      opt.push(
        template(
          "text",
          "值的分隔符",
          `${main.item?.name}_valueSplit`,
          main.option.valueSplit,
          (e) =>
            setMainItems((v) => {
      ...
      opt.push(
        template(
          "radio",
          "值类型",
          `${main.item?.name}_valueType`,
          main.option.valueType,
          (e) =>
            setMainItems((v) => {
        ...
        option: {
          ...
          onClick: () => {
            const confirm = Modal.confirm({
              ...
              content: (
                <FormDesign
                  initValue={{ items: main.list?.items }}
                  onFinish={(v) => {
                    main.list!.items = v.items;
                    confirm.destroy();
                  }}
                />
              ),
...
const template = (
  ...
  return {
    type,
    item: { label, name, initialValue, valuePropName },
    option,
  } as ZeFormItem;
};
